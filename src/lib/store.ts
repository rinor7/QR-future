...
function toRow(data: Partial<CreateQRContact>) {
  return {
    ...
    ...(data.phones !== undefined && { phone: JSON.stringify(data.phones.filter((p) => p.number)) }),
    ...(data.emails !== undefined && { email: JSON.stringify(data.emails.filter((e) => e.email)) }),
    ...(data.websites !== undefined && { website: JSON.stringify(data.websites.filter((w) => w.url)) }),
