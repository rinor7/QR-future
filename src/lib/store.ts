...
function toRow(data: Partial<CreateQRContact>) {
  return {
    ...
    ...(data.phones !== undefined && { phone: (() => { const f = data.phones.filter((p) => p.number); return f.length > 0 ? JSON.stringify(f) : null; })() }),
    ...(data.emails !== undefined && { email: (() => { const f = data.emails.filter((e) => e.email); return f.length > 0 ? JSON.stringify(f) : null; })() }),
    ...(data.websites !== undefined && { website: (() => { const f = data.websites.filter((w) => w.url); return f.length > 0 ? JSON.stringify(f) : null; })() }),
