...
function toContact(row: Record<string, unknown>): QRContact {
  return {
    ...
    phone: (() => { const p = (row.phone as string) ?? ""; try { const a = JSON.parse(p); return Array.isArray(a) ? (a[0] ?? "") : p; } catch { return p; } })(),
    phones: (() => { const p = (row.phone as string) ?? ""; try { const a = JSON.parse(p); return Array.isArray(a) ? a.filter(Boolean) : (p ? [p] : []); } catch { return p ? [p] : []; } })(),
...
function toRow(data: Partial<CreateQRContact>) {
  return {
    ...
    ...(data.phones !== undefined && { phone: JSON.stringify(data.phones.filter(Boolean)) }),
