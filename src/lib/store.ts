...
function toContact(row: Record<string, unknown>): QRContact {
  return {
    ...
    phones: (() => { const p = (row.phone as string) ?? ""; try { const a = JSON.parse(p); if (!Array.isArray(a)) return p ? [{ number: p, label: "" }] : []; return a.map((x: unknown) => typeof x === "string" ? { number: x, label: "" } : (x as { number: string; label: string; kind?: "mobile" | "phone" })); } catch { return p ? [{ number: p, label: "" }] : []; } })(),
    email: (() => { const e = (row.email as string) ?? ""; try { const a = JSON.parse(e); if (!Array.isArray(a)) return e; const f = a[0]; return typeof f === "string" ? f : (f?.email ?? ""); } catch { return e; } })(),
    ...
    website: (() => { const w = (row.website as string) ?? ""; try { const a = JSON.parse(w); if (!Array.isArray(a)) return w; const f = a[0]; return typeof f === "string" ? f : (f?.url ?? ""); } catch { return w; } })(),
...
function toRow(data: Partial<CreateQRContact>) {
  return {
    ...
    ...(data.phones !== undefined && { phone: (() => { const f = data.phones.filter((p) => p.number); return f.length > 0 ? JSON.stringify(f) : ""; })() }),
    ...(data.emails !== undefined && { email: (() => { const f = data.emails.filter((e) => e.email); return f.length > 0 ? JSON.stringify(f) : ""; })() }),
    ...(data.websites !== undefined && { website: (() => { const f = data.websites.filter((w) => w.url); return f.length > 0 ? JSON.stringify(f) : ""; })() }),
    ...
    ...(data.leadCaptureEnabled !== undefined && { lead_capture_enabled: data.leadCaptureEnabled }),
