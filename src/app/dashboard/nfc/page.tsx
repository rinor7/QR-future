...
function ContactPicker({
  ...
  const { tr } = useLang();
  const ph = placeholder ?? tr.nfc_search_contact_ph;
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-white dark:bg-[#1a1d27] border border-slate-200 dark:border-[#242736] rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="max-h-52 overflow-y-auto">
            {filtered.length === 0 ? (
              <p className="px-3 py-4 text-sm text-slate-400 text-center">{tr.nfc_no_contacts}</p>
            ) : (
              filtered.map((c) => {
  ...
  return (
    <div className="pt-8 pb-12 px-4 sm:px-6 lg:px-10 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8 sm:mb-10 gap-4 flex-wrap">
        <div>
          <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">{tr.nfc_title}</h2>
          <p className="text-slate-400 mt-2">{tr.nfc_subtitle}</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: tr.nfc_total_cards, value: cards.length, icon: "nfc", color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-900/20" },
          { label: tr.nfc_assigned, value: assigned.length, icon: "link", color: "text-green-600", bg: "bg-green-50 dark:bg-green-900/20" },
          { label: tr.nfc_unassigned, value: unassigned.length, icon: "link_off", color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-900/20" },
      ...
      {showRegister && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#1a1d27] rounded-3xl shadow-2xl w-full max-w-md p-8">
            <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-1">{tr.nfc_register}</h3>
            <p className="text-sm text-slate-400 mb-6">{tr.nfc_register_hint}</p>
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">{tr.nfc_card_uid}</label>
                <input
                  type="text"
                  value={newUid}
                  onChange={(e) => setNewUid(e.target.value.toUpperCase())}
                  placeholder={tr.nfc_uid_ph}
                  required
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {tr.nfc_chip_hint} <span className="font-mono text-blue-600">{baseUrl}/nfc/{newUid || "CARD-UID"}</span>
                </p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">{tr.nfc_label_optional}</label>
                <input
                  type="text"
                  value={newLabel}
                  onChange={(e) => setNewLabel(e.target.value)}
                  placeholder={tr.nfc_label_ph}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">{tr.nfc_assign_profile}</label>
                <ContactPicker
                  contacts={contacts}
                  value={newContact}
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={registerLoading || !newUid.trim()} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold text-sm transition-colors disabled:opacity-50">
                  {registerLoading ? tr.nfc_registering : tr.nfc_register_btn}
                </button>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-slate-100 dark:border-[#242736] p-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-600 dark:text-slate-300 mb-1">{tr.nfc_no_cards}</h3>
          <p className="text-sm text-slate-400 mb-5">{tr.nfc_no_cards_hint}</p>
        ...
        <div className="bg-white dark:bg-[#1a1d27] rounded-2xl border border-slate-100 dark:border-[#242736] overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 dark:border-[#242736] flex items-center justify-between">
            <h3 className="font-semibold text-slate-900 dark:text-slate-100">{cards.length} {cards.length !== 1 ? tr.nfc_cards : tr.nfc_card} {tr.nfc_registered_suffix}</h3>
          </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {editLabelId === card.id ? (
                        <div className="flex items-center gap-1">
                          ...
                          />
                          <button onClick={() => handleSaveLabel(card.id)} className="text-xs text-blue-600 hover:underline">{tr.nfc_save}</button>
                        </div>
                      ) : (
                        <button
                        ...
                        >
                          {card.label ? `"${card.label}"` : tr.nfc_add_label}
                        </button>
                      )}
                    </div>
                    ...
                    {reassignId === card.id ? (
                      <div className="mt-2 flex items-start gap-2">
                        ...
                        >
                          {reassignLoading ? "…" : tr.nfc_save}
                        </button>
                        <button onClick={() => setReassignId(null)} className="text-xs text-slate-400 hover:text-slate-600 transition-colors shrink-0 mt-1.5">{tr.nfc_cancel}</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        {card.contact_id ? (
                          ...
                          <span className="text-xs text-amber-500 font-medium">{tr.nfc_not_assigned}</span>
                        )}
                        <button
                          onClick={() => { setReassignId(card.id); setReassignContact(card.contact_id ?? ""); }}
                          className="text-xs text-blue-600 hover:underline"
                        >
                          {card.contact_id ? tr.nfc_reassign : tr.nfc_assign}
                        </button>
                      </div>
                    )}
                  </div>
